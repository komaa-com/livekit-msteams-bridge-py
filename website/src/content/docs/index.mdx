...
- **Data-topic context** - `msteams.context` (participant counts, DTMF, recording state), `msteams.goodbye` (the governor's goodbye line for your agent to speak) and `msteams.vision` (opt-in: the caller's screen-share/camera as attributed images).
